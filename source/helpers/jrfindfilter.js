...
class JrFindFilter {
	...
	static getSingleton(...args) {
		...
		if (this.globalSingleton === undefined) {
			this.globalSingleton = new JrFindFilter(...args);
		}
	...
	buildMongooseQueryFromReq(filterOptions, schema, req, jrResult) {
		...
		fieldFilterKeys.forEach((fieldFilterKey) => {
			...
			aFindFilter = this.convertReqQueryStringToAMongooseFindFilter(fieldFilterKey, fieldSchema, fieldFilters[fieldFilterKey], jrResult);
	...
	//---------------------------------------------------------------------------
	convertReqQueryStringToAMongooseFindFilter(fkey, fieldSchema, querystr, jrResult) {
		...
		if (schemaType === Number) {
			...
			retQuery = this.convertReqQueryStringToAMongooseFindFilterNumeric(fkey, schemaType, querystr, "integer", jrResult);
			...
			retQuery = this.convertReqQueryStringToAMongooseFindFilterStringic(fkey, schemaType, querystr, "string", jrResult);
		} else if (schemaType === Date) {
			// it's a date column
			// filter rules:
			//  similar to numeric column
			retQuery = this.convertReqQueryStringToAMongooseFindFilterNumeric(fkey, schemaType, querystr, "date", jrResult);
		} else if (schemaType === mongoose.Schema.ObjectId) {
			// exact match
			retQuery = this.convertReqQueryStringToAMongooseFindFilterStringic(fkey, schemaType, querystr, "idstring", jrResult);
	...
	//---------------------------------------------------------------------------
	convertReqQueryStringToAMongooseFindFilterNumeric(fkey, schemaType, querystr, subType, jrResult) {
		var valPat;
		var mongoValFunc;
	...
	convertReqQueryStringToAMongooseFindFilterStringic(fkey, schemaType, querystr, subType, jrResult) {
		var valPat;
		var mongoValFunc;
	...
	convertReqQueryStringToAMongooseFindFilterMongoStrCmp(strVal, jrResult) {
	...
	convertReqQueryStringToAMongooseFindFilterGenericOperator(fkey, schemaType, querystr, operators, opChars, valPat, mongoValFunc, standaloneOpString, jrResult) {
				...
				if (opChars) {
					str = str.replace(opRegex, (foundstr, g1, g2) => {
						...
						if (mongoOp !== undefined) {
							var obj = this.convertReqQueryStringToAMongooseFindFilterGenericOperatorResolveVal(fkey, g2, mongoOp, mongoValFunc, jrResult);
							andSet.push(obj);
						} else {
	...
	convertReqQueryStringToAMongooseFindFilterGenericOperatorResolveVal(fkey, opVal, mongoOp, mongoValFunc, jrResult) {
		var opValm;
...
module.exports = JrFindFilter.getSingleton();
