...
class RegistrationAid {
	...
	async fillReqBodyWithSessionedFieldValues(req) {
		...
		const login = await arserver.getLoggedInLogin(req);
		if (login) {
		...
		let verification = await arserver.getLastSessionedVerification(req);
	...
	async processAccountAllInOneForm(req, jrResult) {
		...
		const verification = await this.getValidNewAccountVerificationFromRequestOrLastSession(req);
		...
		if (verification && flagVerifiedEmail) {
			...
			const userId = arserver.getLoggedInLocalUserIdFromSession(req);
			const loginId = arserver.getLoggedInLocalLoginIdFromSession(req);
	...
	async createFullNewUserAccountForLoggedInUser(jrResult, req, verification, userObj) {
		...
		// get logged in user
		const loggedInUser = await arserver.getLoggedInUser(req);
		...
		const user = await UserModel.validateAndSaveNew(jrResult, options, true, loggedInUser, userObj, saveFields, ["passwordHashed"], ignoreFields);
